...
cuspath_out = 'CUSpath_out.txt'
output_file = open(cuspath_out,'w')
output_file.close
...
class SingleLinkedList:
    ...
    def append(self, release_time,period,remain_execution_time,absolute_deadline,tid):
        ...
        #比較新job跟ready queue中job的absolutedeadline
            ...
            print(new_node.tid)
            print(new_node.absolute_deadline)
        ...
        if self.head == None:
            ...
            while current_node != None:
                if(current_node.absolute_deadline-Clock-current_node.remain_execution_time<0):
                    with open(cuspath_out,'a') as output_file:
...
while(Clock<(MaxSimTime)):
    ...
    if(cus_deadline==Clock):
        if(len(queue_aperiodic)>0):
            ...
            cus_deadline = Clock + queue_aperiodic[0][2]/us
            queue_aperiodic.pop(0)

    #檢查目前時間是否是aperiodic job的抵達時間
    for i in range(len(task_aperiodic)):
        if(Clock == task_aperiodic[i][1]):
            #假設aperiodic沒有job且clock大於cus的絕對截限時間，則更新cus的絕對截限時間
            if(len(ready_queue_aperiodic)==0 and task_aperiodic[i][1]>=cus_deadline):
                cus_deadline = Clock + task_aperiodic[i][2]/us
                task_pair = []
