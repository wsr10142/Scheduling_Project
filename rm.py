...
schedule = []
path_in = 'test2.txt'
RMpath_out = 'RMpath_out.txt'
output_file = open(RMpath_out, 'w')
output_file.close
...
class SingleLinkedList:
    ...
    def append(self, release_time, period, remain_execution_time, absolute_deadline, tid):
        # 建立Job的新Node
        new_node = Job(release_time, period,
                       remain_execution_time, absolute_deadline, tid)
        ...
        #比較新job跟ready queue中job的period
        else:
            current_node = self.head
            while current_node != None:
                #當新工作的週期較小
                if(new_node.period < current_node.period):
                    if(len(self) == 1):
                        self.head = new_node
                        ...
                        if(current_node == self.head):
                            ...
                            return
                #當週期一樣時，判斷編號大小
                elif(new_node.period==current_node.period):
    ...
    def checkAbsolutedeadline(self, Clock):
        ...
        if self.head == None:
            ...
            current_node = self.head
            while current_node != None:
                print(current_node.absolute_deadline)
                print(Clock)
                print(current_node.remain_execution_time)
                if(current_node.absolute_deadline-Clock-current_node.remain_execution_time < 0):
                    print(str(current_node.absolute_deadline-Clock-current_node.remain_execution_time))
                    with open(RMpath_out, 'a') as output_file:
...
def Schedulability_test():
    ...
    print(sum)
    if(sum <= len(task)*(2**(1/len(task))-1)):
        print(len(task)*(2**(1/len(task))-1))
        with open(RMpath_out, 'a') as output_file:
...
while(Clock < (LCM+MaxPH)):
    ...
    # 檢查missdeadline的工作
    Miss_Deadline_Job_Number+=ready_queue.checkAbsolutedeadline(Clock)
