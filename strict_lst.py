...
schedule = []
path_in = 'test2.txt'
strict_LSTpath_out = 'strict_LSTpath_out.txt'
output_file = open(strict_LSTpath_out,'w')
output_file.close

#ready queue
class Job:
    def __init__(self,release_time,slack,remain_execution_time,absolute_deadline,tid):
...
class SingleLinkedList:
    ...
    def append(self, release_time,slack,remain_execution_time,absolute_deadline,tid):
        #建立Job的新Node
        new_node = Job(release_time,slack,remain_execution_time,absolute_deadline,tid)
        ...
        #比較新job跟ready queue中job的slack
        else:
            current_node = self.head
            new_node_slack = new_node.slack
            while current_node != None:
                current_node_slack = current_node.slack
                if(new_node_slack<current_node_slack):
                    if(len(self)==1):
                        self.head = new_node
                        ...
                        if(current_node==self.head):
                            ...
                            return
                #當slack一樣時，判斷編號大小
                elif(new_node_slack==current_node_slack):
    ...
    #計算ready queue中的每一個工作的slack       
    def detemineSlack(self,Clock):
        current_node = self.head
        while current_node != None:
            current_node.slack = current_node.absolute_deadline-Clock-current_node.remain_execution_time
            print(current_node.tid)
            print(current_node.slack)
            current_node = current_node.next
        return
        ...
        if self.head == None:
            ...
            while current_node != None:
                if(current_node.absolute_deadline-Clock-current_node.remain_execution_time<0):
                    with open(strict_LSTpath_out,'a') as output_file:
    ...
    def schedule(self,Clock):
        if self.head == None:
            ...
            high_priority_node = self.head
            if(len(self)>1):
                while current_node != None:
                    if(current_node.slack<min):
                        high_priority_node = current_node
                        min = current_node.slack
                        current_node = current_node.next
                    elif(current_node.slack==min):
                        if(int(current_node.tid[1])<int(high_priority_node.tid[1])):
                            high_priority_node = current_node
                            min = current_node.slack
                            current_node = current_node.next
                        else:
            ...
            tid = high_priority_node.tid
            high_priority_node.remain_execution_time-=1
            if(high_priority_node.remain_execution_time==0):
                complete_job = 1
                if(len(self)==1):
                    self.head = None
                    self.tail = None
                else:
                    if(high_priority_node==self.head):
                        self.head = self.head.next
                    elif(high_priority_node==self.tail):
                        index = self.head
                        while index.next != self.tail:
                            index = index.next
                        index.next = None
                        ...
                        index = self.head
                        while index.next != high_priority_node:
                            index = index.next
                        index.next = high_priority_node.next
        with open(strict_LSTpath_out,'a') as output_file:
...
while(Clock<(LCM+MaxPH)):
    ...
    for i in range(len(task)):
        if((Clock - task[i][1]) % task[i][2] == 0 and Clock>=task[i][1]):
            ready_queue.append(Clock,(Clock+task[i][3])-Clock-task[i][4],task[i][4],Clock+task[i][3],task[i][0])
            Total_Job_Number+=1
    ...
    ready_queue.detemineSlack(Clock) #計算工作的slack
